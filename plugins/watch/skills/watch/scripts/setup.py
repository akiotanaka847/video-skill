...
- SETUP_COMPLETE=true in ~/.config/video-skill/.env tells us the user has been
...
from config import CONFIG_DIR, CONFIG_FILE, LEGACY_CONFIG_FILE, get_config  # noqa: E402
...
# Caption languages to request, in preference order. The first one that the
# source actually publishes wins. Free captions beat paid Whisper, so list
# every language you realistically watch.
WATCH_SUB_LANGS=es,en

# Force the Whisper language (ISO-639-1) instead of auto-detecting. Worth
# setting when your audio is one language carrying loanwords from another —
# auto-detect flips mid-file on exactly that material.
# WATCH_WHISPER_LANG=es

# Domain vocabulary fed to Whisper as a decoding prior. Comma-separated terms,
# no instructions: the model conditions on this text as if it preceded the
# audio, which is what stops proper nouns coming back mangled.
# WATCH_GLOSSARY=Workfront, Adobe, Qaio, OpenKAG, Claude Code

# Cache transcripts under ~/.cache/video-skill so re-watching a video is free
# and instant. Set to false to disable.
# WATCH_CACHE=true

# Default destination for --save notes (e.g. /path/to/repo/reports).
# WATCH_NOTES_DIR=
"""
...
def _scaffold_env() -> bool:
    """Create ~/.config/video-skill/.env with placeholders if missing."""
