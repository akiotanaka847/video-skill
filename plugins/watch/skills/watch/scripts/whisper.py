...
def _post_whisper(
    ...
    # ISO-639-1 hint. Auto-detect is decent on clean speech but flips language
    # mid-file on Spanish audio carrying English technical vocabulary, which is
    # exactly the common case here. Forcing it is cheaper than re-running.
    if language:
        fields["language"] = language
    # Whisper's `prompt` is a decoding prior, not an instruction: the model
    # conditions on it as if it were the text preceding this audio. A bare
    # comma-separated term list is the effective shape.
    if prompt:
...
def _transcribe_file(
    ...
    if backend == "groq":
        response = _post_whisper(
            GROQ_ENDPOINT, api_key, GROQ_MODEL, audio_path, language, prompt
        )
    elif backend == "openai":
        response = _post_whisper(
            OPENAI_ENDPOINT, api_key, OPENAI_MODEL, audio_path, language, prompt
        )
    else:
...
def transcribe_video(
    ...
    if not backend or not api_key:
        ...
        raise SystemExit(
            ...
            "in the environment or in ~/.config/video-skill/.env. "
            f"Run `python3 {setup_py}` to configure."
        )

    hint = f" lang={language}" if language else " lang=auto"
    hint += " +glossary" if prompt else ""
    print(f"[watch] extracting audio for Whisper ({backend}{hint})…", file=sys.stderr)
    audio_path = extract_audio(video_path, audio_out)
    audio_bytes = audio_path.stat().st_size

    def transcribe_one(path: Path) -> list[dict]:
        # language/prompt are re-sent for every chunk on purpose: each chunk is
        # transcribed in isolation, so a glossary sent only with chunk 1 would
        # stop biasing the decode from chunk 2 onward.
        return _transcribe_file(backend, api_key, path, language, prompt)
